...
class MovieRepositoryMocked : Repository<Movie> {
    ...
    override suspend fun getById(id: String) = movieList.filter { it.id == id }[0]

    override suspend fun search(searchString: String) = movieList.filter { it.name.contains(searchString) }
}